...
- No semicolons (enforced by project style — `useAudioRecorder.ts` is the exception, copied from external source)
...
- `web/.env` — `VITE_DEBUG`, `VITE_API_URL`
- `backend/.env` — `PORT`, `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID`, `XAI_API_KEY`
