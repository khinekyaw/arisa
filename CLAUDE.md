...
- Voice providers are toggleable per-channel via `TTS_PROVIDER`/`STT_PROVIDER` (`xai` default, `elevenlabs` fallback). STT dispatch in `voiceRoute.ts`; realtime STT in `transcribeSocket.ts`. The browser always speaks the ElevenLabs-style WS protocol (`input_audio_chunk` ⇄ `partial_transcript`/`committed_transcript`); for xAI the proxy translates that to/from xAI's binary frames + `transcript.partial`/`transcript.done`, so the frontend is provider-agnostic — don't add provider logic to `useStreamingTranscription.ts`
- TTS is STREAMED for low latency, not returned inline. `POST /api/chat` returns fast after the LLM with an `audio_token` (no audio bytes); the spoken text is stashed in `services/ttsTokens.ts`. The browser's `<audio>` element hits `GET /api/tts/:token`, which pipes audio as it generates: xAI via its streaming TTS WS (`streamXaiTts`), ElevenLabs via a one-shot buffer. xAI streaming gotcha: the WS URL must carry ONLY `language`/`voice`/`codec` — adding `sample_rate`/`bit_rate` makes it silently return no audio (output is 24kHz mp3); messages use the `delta` field, not `text`/`audio`. The audio element sets `crossOrigin="anonymous"` so wawa-lipsync can analyze it cross-origin in dev — relies on the `/api` CORS headers (`CLIENT_ORIGIN`)
...
- iOS Safari quirks (don't regress): (1) `getUserMedia` needs HTTPS — fails on plain http; (2) iOS ignores `AudioContext({sampleRate:16000})` and records at the hardware rate, so the client sends its real `ctx.sampleRate` per chunk and `bridgeXai` opens the xAI STT socket lazily with THAT rate (never hardcode 16k upstream); (3) `AudioContext` starts suspended and only resumes inside a user gesture — the mic context resumes in `start()`, and the lip-sync/TTS context resumes via `unlockAudioPlayback()` called from the mic/send tap handlers in `Chat.tsx`. Also uses the `webkitAudioContext` fallback
